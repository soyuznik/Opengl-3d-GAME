...
#include "../includes/learnopengl/shader.h"
...
public:
	...
	void load_text(GLFWwindow* WINDOW, std::string font_name); // loads text from a .tff file
	void drawText(std::string text, float x, float y, float scale, glm::vec3 color); // draws the text , no DRAW(N) neccesray
	Text(GLFWwindow* window, std::string path); // constructor which calls load_text
};
